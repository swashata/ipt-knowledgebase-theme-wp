...
class IPT_Bootstrap_Walker_Nav_Menu extends Walker_Nav_Menu {
	...
	function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if (
			...
			// Add icon
			if ( isset( $item->icon ) && '' !== $item->icon && 'none' != $item->icon ) {
				$item_output .= ' <i class="ipticm" data-ipt-icomoon="&#x' . dechex( $item->icon ) . ';"></i> ';
			}
		...
		if ( in_array( 'menu-item-has-children', $classes ) && $depth == 0 ) {
			...
			$item_output .= '<a href="#" data-toggle="dropdown" class="dropdown-toggle" aria-haspopup="true"><i class="ipticm ipt-icomoon-caret-down"></i></a>';
		}
